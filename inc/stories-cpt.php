...
/**
 * Resolve a thumbnail image URL for a story video URL, used by both the
 * Stories widget and the Universal Video widget. Runs on every render, so
 * anything requiring a remote call (TikTok) is transient-cached.
 */
function cbk_story_get_video_thumbnail( $url ) {
    if ( empty( $url ) ) {
        return '';
    }
